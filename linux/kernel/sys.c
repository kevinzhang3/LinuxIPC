//eSPDX-License-Identifier: GPL-2.0
/*
 *  linux/kernel/sys.c
 *
...
/*        CMPT 432/832 CHANGE BY zbk618 job346, 2025, 1, 27  */
#include <linux/block_send.h>
/*        end CMPT 432/832 CHANGE BY zbk618 job346, 2025, 1, 27  */
...
/*        CMPT 432/832 CHANGE BY zbk618 job346, 2025, Jan, 26  */

SYSCALL_DEFINE5(pSend, pid_t, to, void*, sData, unsigned int, slen, 
        void*, rData, unsigned int*, rlen) {
    if (pSend(to, sData, slen, rData, rlen) >= 0) {
        return 0;
    } else {
        return -EFAULT;
    }
...
SYSCALL_DEFINE3(pReceive, pid_t*, to, void*, Data, unsigned int*, len) {
    if (pReceive(to, Data, len) >= 0) {
        return 0;
    } else {
        return -EFAULT;
    }
...
SYSCALL_DEFINE3(pReply, pid_t, sndr, void*, Data, unsigned int, len) {
    if (pReply(sndr, Data, len) >= 0) {
        return 0;
    } else {
        return -EFAULT;
    }
...
/*       end CMPT 432/832 CHANGE BY zbk618 job346, 2025, Jan, 26  */
